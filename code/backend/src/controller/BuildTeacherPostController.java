...
public class BuildTeacherPostController implements HttpHandler {
    private UserService userService;
    private MySQLRepository repository;

    public BuildTeacherPostController() {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        if (exchange.getRequestMethod().equalsIgnoreCase("OPTIONS")) {
            ...
            try {
                ...
                boolean buildSuccess = userService.buildTeacherPost(post, repository);

                if (buildSuccess) {
                    String responseMessage = "Build teacher post successfully!";
                    sendResponse(exchange, 200, responseMessage);
                } else {
                    String responseMessage = "Error occurred during teacher pos build.";
