...
public class TeacherInputWorkExperienceController implements HttpHandler {
    private UserService userService;
    private MySQLRepository repository;

    public TeacherInputWorkExperienceController() {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        if (exchange.getRequestMethod().equalsIgnoreCase("OPTIONS")) {
            ...
            try {
                ...
                String workingexperience = jsonObject.get("workingexperience").getAsString();

                Teacher teacher = new Teacher();
                teacher.setWorkExperience(workingexperience);

                // Attempt to build the contract
                boolean inputSuccess = userService.teacherinputworkingexperience(teacher,repository);

                if (inputSuccess) {
                    String responseMessage = "Input work experience successfully!";
                    sendResponse(exchange, 200, responseMessage);
                } else {
                    String responseMessage = "Error occurred during input work experience.";
