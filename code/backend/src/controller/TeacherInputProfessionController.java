...
public class TeacherInputProfessionController implements HttpHandler {
    private UserService userService;
    private MySQLRepository repository;

    public TeacherInputProfessionController() {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        if (exchange.getRequestMethod().equalsIgnoreCase("OPTIONS")) {
            ...
            try {
                ...
                boolean inputSuccess = userService.teacherinputprofession(teacher,repository);

                if (inputSuccess) {
                    String responseMessage = "Input profession successfully!";
                    sendResponse(exchange, 200, responseMessage);
                } else {
                    String responseMessage = "Error occurred during input profession.";
