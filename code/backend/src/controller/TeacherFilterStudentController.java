...
public class TeacherFilterStudentController implements HttpHandler {
    private UserService userService;
    private MySQLRepository repository;

    public TeacherFilterStudentController() {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        if (exchange.getRequestMethod().equalsIgnoreCase("OPTIONS")) {
            ...
            try {
                ...
                List<Post> filteredPosts = userService.filterStudentsByCriteria(subject, gender, mbti, region, repository);
                if (filteredPosts != null) {
                    // Convert the filtered posts to JSON and send as the response
                    String jsonResponse = gson.toJson(filteredPosts);
                    sendResponse(exchange, 200, jsonResponse);
                } else {
                    String responseMessage = "Error occurred during filtering students' posts.";
